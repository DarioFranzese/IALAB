...
checkAvversario(_, R, C, [martello(pos(_, _)) | _ ]):- ghiaccio(pos(R, C)). %E' UN CASO BLOCCANTE SOLO SE NON HO IL MARTELLO.
...
checkAvversario(_, R, C, Movibili):- member(avversario(pos(R, C)), Movibili), fail, !. %da controlalre se il cut va prima


%CHIAMATE RICORSIVE PER PROSEGUIRE LA RICERCA
checkAvversario(nord, R, C, Movibili):- NR is R-1, checkAvversario(nord, NR, C, Movibili).
checkAvversario(sud, R, C, Movibili):- NR is R+1, checkAvversario(sud, NR, C, Movibili).
checkAvversario(est, R, C, Movibili):- NC is C+1, checkAvversario(est, R, NC, Movibili).
checkAvversario(ovest, R, C, Movibili):- NC is C-1, checkAvversario(ovest, R, NC, Movibili).
